...
( function( wp, $ ) {
	...
	wp.customize( 'custom_logo', function( setting ) {
		setting.bind( function( value ) { 
			if ( '' !== value ) {
				...
				wp.customize.instance( 'dynamico_theme_options[retina_logo]' ).set( false );
			}
		});
		...
		wp.customize.control( 'dynamico_theme_options[retina_logo_title]', setupControl );
		wp.customize.control( 'dynamico_theme_options[retina_logo]', setupControl );
	} );

	// Based on https://make.xwp.co/2016/07/24/dependently-contextual-customizer-controls/
	wp.customize( 'dynamico_theme_options[blog_content]', function( setting ) {
		var setupControl = function( control ) {
			var setActiveState, isDisplayed;
			isDisplayed = function() {
		...
		wp.customize.control( 'dynamico_theme_options[excerpt_length]', setupControl );
		wp.customize.control( 'dynamico_theme_options[excerpt_more_text]', setupControl );
	} );

	// Based on https://make.xwp.co/2016/07/24/dependently-contextual-customizer-controls/
	wp.customize( 'dynamico_theme_options[featured_posts]', function( setting ) {
		var setupControl = function( control ) {
			var setActiveState, isDisplayed;
			isDisplayed = function() {
		...
		wp.customize.control( 'dynamico_theme_options[featured_category]', setupControl );
		wp.customize.control( 'dynamico_theme_options[featured_layout]', setupControl );
	} );
	 ...
	 */
	wp.customize.controlConstructor.dynamico_custom_font = wp.customize.Control.extend({
		/**
